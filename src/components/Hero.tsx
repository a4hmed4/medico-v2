import React from 'react';
import { ArrowRight, CheckCircle2, Stethoscope, Brain, Camera } from 'lucide-react';

const Hero: React.FC = () => {
  return (
    <>
      <div className="relative overflow-hidden bg-white dark:bg-gray-900 pt-16">
        <div className="max-w-7xl mx-auto">
          <div className="relative z-10 pb-8 sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32">
            <main className="mt-10 mx-auto max-w-7xl px-4 sm:mt-12 sm:px-6 md:mt-16 lg:mt-20 lg:px-8 xl:mt-28">
              <div className="sm:text-center lg:text-left">
                <h1 className="text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white sm:text-5xl md:text-6xl">
                  <span className="block">Revolutionizing Healthcare</span>
                  <span className="block text-blue-600">with Medico</span>
                </h1>
                <p className="mt-3 text-base text-gray-500 dark:text-gray-400 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
                  Medico is an AI-powered mobile application that provides accurate symptom-based diagnoses
                  and X-ray analysis, ensuring accessible and efficient healthcare solutions for users anytime, 
                  anywhere.
                </p>
                <div className="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
                  <div className="rounded-md shadow">
                    <a
                      href="https://www.mediafire.com/file/your-app-file/app.apk"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 md:py-4 md:text-lg md:px-10"
                    >
                      Get start now
                      <ArrowRight className="ml-2 h-5 w-5" />
                    </a>
                  </div>
                </div>
              </div>
            </main>
          </div>
        </div>
        <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2">
          <img
            className="h-56 w-full object-cover sm:h-72 md:h-96 lg:w-full lg:h-full"
            src="https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80"
            alt="Healthcare professionals using digital technology"
          />
        </div>
      </div>

      {/* How It Works Section */}
      <div className="bg-gray-50 dark:bg-gray-800 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white sm:text-4xl">
              How It Works
            </h2>
            <p className="mt-4 text-lg text-gray-500 dark:text-gray-400">
              Experience healthcare at your fingertips with our innovative features
            </p>
          </div>

          <div className="mt-16">
            <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
              {/* Feature 1 */}
              <div className="relative p-6 bg-white dark:bg-gray-700 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105">
                <div className="flex items-center justify-center h-12 w-12 rounded-md bg-blue-500 text-white mb-4">
                  <Stethoscope className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">Symptom Analysis</h3>
                <p className="mt-2 text-base text-gray-500 dark:text-gray-400">
                  Get instant AI-powered diagnosis based on your symptoms
                </p>
              </div>

              {/* Feature 2 */}
              <div className="relative p-6 bg-white dark:bg-gray-700 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105">
                <div className="flex items-center justify-center h-12 w-12 rounded-md bg-blue-500 text-white mb-4">
                  <Camera className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">X-ray Analysis</h3>
                <p className="mt-2 text-base text-gray-500 dark:text-gray-400">
                  Upload X-ray images for instant AI analysis and interpretation
                </p>
              </div>

              {/* Feature 3 */}
              <div className="relative p-6 bg-white dark:bg-gray-700 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105">
                <div className="flex items-center justify-center h-12 w-12 rounded-md bg-blue-500 text-white mb-4">
                  <Brain className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">AI-Powered Insights</h3>
                <p className="mt-2 text-base text-gray-500 dark:text-gray-400">
                  Get detailed medical insights powered by advanced AI technology
                </p>
              </div>
            </div>

            {/* Download Button */}
            <div className="mt-12 text-center">
              <a
                href="https://www.mediafire.com/file/your-app-file/app.apk"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transform transition-all duration-300 hover:scale-105 hover:shadow-lg"
              >
                Download App Now
                <ArrowRight className="ml-2 h-5 w-5" />
              </a>
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
                Available for Android devices
              </p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Hero;